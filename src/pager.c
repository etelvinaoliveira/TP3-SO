    ...
    if (my_pager.blocks_free>0)
    {
        ...
        for (int i = 0; i < my_pager.n_procs; i++)
        {
            if (my_pager.pid2proc[i].pid == pid)
            {
                ...
                my_pager.pid2proc[i].pages[ind_page].on_disk = 1;
                break;
            }
        }
    }
    ...
    for (int i = 0; i < my_pager.n_procs; i++)
    {
        if (my_pager.pid2proc[i].pid==pid)
        {
            ...
            if (my_pager.pid2proc[i].pages[page].on_disk) //não está na RAM
            {
                ...
                {
                    my_pager.frames_free--;
                    my_pager.pid2proc[i].pages[page].frame = my_pager.free_frames_stack[my_pager.frames_free];
                    frame = my_pager.frames_free;
                }
                else
                {
                    second_chance();
